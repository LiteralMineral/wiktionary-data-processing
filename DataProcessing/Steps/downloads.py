...
def collect_file(lang: string):
    ...
    if lang == "All":
        ...
        print(
            f"File is too large to download nicely. Download it yourself and name it {lang}_kaikki_data.jsonl"
        )
    else:
...
def assign_ids(spark: SparkSession, lang: string):
    ...
    df.write.mode("overwrite").parquet(
        f"{settings.dir_info['data_proc']}/has_id_column/{lang}"
    )
    return df
...
def save_basic_info(spark: SparkSession, lang, data: DataFrame, url, db_props):
    df = data[
        [f.name for f in data.schema.fields if not isinstance(f.dataType, ArrayType)]
    ]
    # df.write.option('url', url).option('dbtable', True).saveAsTable(name=f"{lang}_demo_basic_info",
    #                      format='jdbc', mode="overwrite")
    df.write.mode("overwrite").jdbc(
        url, table=f"wiktionary_info.{lang}_basic_info", properties=db_props
    )
...
def convert_all_to_json(data: DataFrame):
    ...
    df = reduce(
        lambda memo_df, col_name: memo_df.withColumn(col_name, funcs.to_json(col_name)),
        data.columns,
        data,
    )
...
def convert_some_to_binary(data: DataFrame):
    ...
    df = reduce(
        lambda memo_df, col_name: memo_df.withColumn(
            col_name, funcs.encode(col_name, "utf-8")
        ),
        columns,
        data,
    )
...
def convert_all_from_json(data: DataFrame):
    df = reduce(
        lambda memo_df, col_name: memo_df.withColumn(
            col_name, funcs.from_json(col_name)
        ),
        data.columns,
        data,
    )
...
def get_all_json_schema(data: DataFrame):
    df = reduce(
        lambda memo_df, col_name: memo_df.withColumn(
            col_name, funcs.schema_of_json(col_name)
        ),
        data.columns,
        data,
    )
