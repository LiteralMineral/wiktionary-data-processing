...
from pyspark.sql import \
    SparkSession, \
    DataFrame, \
    functions as funcs, Column, DataFrameReader
...
def join_language_data(langs, filename, spark: SparkSession):
    tables = [spark.read.parquet(f"{settings.dir_info['data_proc']}/{filename}/{lang}")
              for lang in langs]
    for tab in tables:
        print(tab.columns)
...
def build_collective_data(langs: typing.List[str], filename: str, spark_read_func = types.FunctionType) -> DataFrame:
    result = reduce(lambda acc_df, lang: acc_df.union(load_dataset(lang, filename, spark_read_func)),
...
def load_datasets(langs, filename, spark_read_func: types.FunctionType) -> typing.List[DataFrame]:
    tables = [spark_read_func(f"{settings.dir_info['data_proc']}/{filename}/{lang}")
              for lang in langs]
    return tables
...
def save_dataset(table: DataFrame, lang: str, filename: str, write_to_parquet: bool = True):
...
def write_to_single_csv(data: DataFrame, lang, filename):
    ...
    output_file = f"{settings.dir_info['data_proc']}/{filename}/{lang}/{lang}_{filename}.csv"
    ...
    data.write.mode("overwrite")\
        ...
        .csv(tmp_output_dir, header=True)
...
def sample_known_tags(lang: str, pos: str, ) -> DataFrame:
    pass
...
def apply_to_data(langs: Iterable[str], filename: str, procedure: typing.Callable[[str, str], None], kwargs) -> None:
    for lang in langs:
...
def apply_to_columns(data: DataFrame, col_names: Iterable[str], func):
    return reduce(lambda acc_df, col_name: acc_df.withColumn(col_name, func(col_name)\
                                                             .alias(col_name)),
                  col_names,
                  data)
...
def sort_tags_column(data: DataFrame, column_name, filter_tags: typing.Dict):
    return reduce(
        lambda acc_df, cat_name:
        acc_df.withColumn(cat_name,                                 # given the category name
                          funcs.array_intersect(data[column_name],  # and the tag column to filter
                                                lit(filter_tags[cat_name]))), # filtering tags
